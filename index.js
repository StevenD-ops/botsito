 /*
* Bender Bot es una creación de Bender Bot
* Bender Bot no tiene ningun fin de lucro
* Bender Bot se reserva todos los derechos de autor
* © 2021 Bender Bot, INC.
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'Sticker BenderBot'; if (!author) author = 'Steven';	
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Conectado by Bender Bot')
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["1809152058@s.whatsapp.net"] // replace this with your number
       ...
       /******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Sticker ws'; if (!author) author = 'steven';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
	...
	case 'hidetag':
			...
			case 'tagall':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 MENCIONANDO 〙✪══\n╠➥'+teks+'╚═〘 by Bender Bot 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
